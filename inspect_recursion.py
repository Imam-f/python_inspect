...
def transform_tail_recursive_function(func):
    """
    Transform a *very specific* tail-recursive function into an iterative version:
    - If-statement or match-statement based.
    
    Returns a NEW function object. The original remains unchanged.
    """
    ...
    new_func.__name__ = func.__name__ + "_iter"
    return new_func
...
if __name__ == "__main__":
    ...
    # EXAMPLE 1: Tail-recursive factorial using `match`
    def factorial(n, acc=1):
        """Tail-recursive factorial using a match statement."""
        match n:
            case 0 | 1:
                return acc
            case _:
    ...
    iter_factorial = transform_tail_recursive_function(factorial)
    print("match-based factorial(5) =", factorial(5))          # 120 (original)
    print("iter_factorial(5) =", iter_factorial(5))            # 120 (transformed)

    # EXAMPLE 2: Traditional tail-recursive factorial using `if`
    def factorial_if(n, acc=1):
        """Tail-recursive factorial using if statement."""
        if n <= 1:
            return acc
        return factorial_if(n - 1, acc * n)

    iter_factorial_if = transform_tail_recursive_function(factorial_if)
    print("if-based factorial(5) =", factorial_if(5))          # 120 (original)
    print("iter_factorial_if(5) =", iter_factorial_if(5))      # 120 (transformed)
